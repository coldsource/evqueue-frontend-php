...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="content">
		...
		<input type="hidden" name="queue_id" value="{/page/response-queue/queue/@id}" />
			...
			<div>
				<label for="queue_concurrency">Queue concurrency:</label>
			...
			<div>
				<label for="queue_scheduler">Queue scheduler:</label>
			...
			<div>
				<label for="queue_dynamic">Dynamic</label>
				<input type="checkbox" name="dynamic" value="1" >
					<xsl:if test="/page/response-queue/queue/@dynamic = 'yes'">
