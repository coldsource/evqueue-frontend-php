			...
			<form id="formWorkflowSchedule" method="POST">
				<div id="tabs" class="">
					...
					<div id="generalTab">
						...
						<input type="hidden" name="workflow_schedule_id" value="{/page/schedule/@id}" />
						<input type="hidden" name="schedule" placeholder="Schedule definition" style="width: 50%;" value="{/page/schedule/schedule}" />
						...
						<div class="infoBloc">
								...
								<xsl:choose>
									<xsl:when test="count(/page/schedule) = 0">
										<xsl:apply-templates select="/page/groups" mode="select_workflow">
											...
											<xsl:with-param name="selected_value" select=" /page/schedule/workflow_id" />
											<xsl:with-param name="value" select="'id'" />
											<xsl:with-param name="display-bound-workflows" select="'no'" />
										</xsl:apply-templates>
									</xsl:when>
									<xsl:otherwise>
										<input type="hidden" name="workflow_id_select" value="{/page/schedule/workflow_id}" />
										<xsl:value-of select="/page/workflows/workflow[@id = /page/schedule/workflow_id]/@name" />
									</xsl:otherwise>
								</xsl:choose>
							</div>
							...
							<div class="item planChoice hideMe" data-group="what">
								<xsl:call-template name="edit-simple-workflow">
									<xsl:with-param name="standalone" sleect="'no'" />
								</xsl:call-template>
							</div>
							
							<xsl:if test="/page/workflows/workflow[@id = /page/schedule/workflow_id]/@bound-to-schedule = 0">
								<script type="text/javascript">
									$('.selectMode[data-group=what] span:eq(1)').remove();
								</script>
							</xsl:if>
							
							<xsl:if test="/page/workflows/workflow[@id = /page/schedule/workflow_id]/@bound-to-schedule = 1">
								<script type="text/javascript">
						...
						<div class="infoBloc">
							...
							<div class="item">
								...
								<select name="onfailure">
									<option value="SUSPEND">
										<xsl:if test="/page/schedule/onfailure = 'SUSPEND'">
											<xsl:attribute name="selected">selected</xsl:attribute>
										</xsl:if>
										Suspend
									</option>
									<option value="CONTINUE">
										<xsl:if test="/page/schedule/onfailure = 'CONTINUE'">
											<xsl:attribute name="selected">selected</xsl:attribute>
										</xsl:if>
										Continue
									</option>
							...
							<div class="item">
								...
								<input type="text" name="schedule_comment" id="schedule_comment" value="{/page/post/@schedule_comment | /page/schedule/comment }" placeholder="" />	
							</div>

							<div class="item">
								<label class="formLabel" for="active">Active : </label>
								<input type="checkbox" name="active" id="active">
									<xsl:if test="/page/schedule/active = 1">
					...
					<div id="hostTab">
						...
						<input type="text" name="schedule_user" id="schedule_user" value="{/page/schedule/user}" placeholder="Enter user here, leave blank for local execution" />

						<br />
						<label class="formLabel" for="schedule_host">Task host : </label>
						<input type="text" name="schedule_host" id="schedule_host" value="{/page/schedule/host}" placeholder="Enter host here, leave blank for local execution" />
					</div>
					<input type="button" value="Save this plan" onclick="setParameter(); $('#formWorkflowSchedule').submit();" />
				</div>
			</form>
			<div id="paramsTabClone">
				<xsl:for-each select="/page/workflows/workflow/workflow">
